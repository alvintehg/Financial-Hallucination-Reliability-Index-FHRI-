# FHRI Robo-Advisor (Full Stack)

Full-stack project with FastAPI backend (FHRI, entropy, NLI, RAG) and React frontend dashboard.
...
```
2) Configure env (use **your own keys**; keys are never stored in repo)
```bash
...
- Add knowledge passages to `data/passages.txt` (blank-line separated).
- Optional trusted docs in `data/trusted_docs/`.
- Models/indices load from `models/` if present; will fallback gracefully if missing.
4) Run API
```bash
...
- Override via `.env` in `frontend/`:
  ```
  REACT_APP_API_BASE_URL=http://your-backend:8000
  ```
3) Tests / build
```bash
npm test
...
## Directory map
```
src/                 # FastAPI app, FHRI, retrieval, NLI
data/                # Passages/trusted docs (optional)
models/              # Saved TF-IDF/FAISS/NLI artifacts (optional)
results/             # Evaluation outputs/plots
scripts/             # Evaluation and tooling scripts
frontend/            # React UI
docs/                # Additional docs/templates
```

## Notes
- Provide your own provider keys (DeepSeek/OpenAI) via backend `.env`.
- Frontend uses `REACT_APP_API_BASE_URL`; no keys in the UI.
- If backend is down, the frontend shows demo fallback messaging.
